...
abstract class IkigaiMangas :
    ...
    override suspend fun fetchMangaUpdate(
        ...
        val updatedManga = SManga.create().apply {
            title = document.selectFirst(".card-body .card-title")!!.text()
            thumbnail_url = document.selectFirst("img")?.attr("abs:src")
            description = document.selectFirst(".card-body > p")?.text()
            status = parseStatus(document.selectFirst("figure > ul a[href*=?estados]")?.text())
            genre = document.select(".card-body > ul > li > a[href*=?generos]").joinToString { it.text().trim() }
        }
        ...
        if (fetchChapters) {
            document.let { doc ->
                val chapters = doc.select("section.card > ul.grid a.card").map(::chapterFromElement)
